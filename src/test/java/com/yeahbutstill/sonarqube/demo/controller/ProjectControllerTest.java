...
public class ProjectControllerTest {
  ...
  public void findAllProject_expectSuccess() throws Exception {
    ...
    Mockito.when(projectService.listProject())
        .thenReturn(
            ResponseEntity.ok()
                .body(
                    BaseResponse.builder()
                        ...
                        .build()));
    mockMvc
        .perform(post("/project/").contentType(MediaType.APPLICATION_JSON))
    ...
    mockMvc
        .perform(
            post("/project/update")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"id\":1,\"project_code\":\"Code01\",\"project_description\":\"Setoran\",\"mandays\":10}"))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/project/update")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"project_code\":\"Code01\",\"project_description\":\"Setoran\",\"mandays\":10}"))
    ...
    mockMvc
        .perform(
            post("/project/create")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"project_code\":\"code125\",\"project_description\":\"TransactionSetoran\",\"mandays\":10}"))
        .andExpect(status().isOk())
  ...
  public void deleteProject_expectSuccess() throws Exception {
    ...
    mockMvc
        .perform(
            post("/project/delete").contentType(MediaType.APPLICATION_JSON).content("{\"id\":1}"))
        .andExpect(status().isOk())
  ...
  public void deleteProject_expectError() throws Exception {
    ...
    mockMvc
        .perform(
            post("/project/delete").contentType(MediaType.APPLICATION_JSON).content("{\"id\":3}"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.message", is(ConstantMess.KEY_DATA_NOT_FOUND)));
  }
}
