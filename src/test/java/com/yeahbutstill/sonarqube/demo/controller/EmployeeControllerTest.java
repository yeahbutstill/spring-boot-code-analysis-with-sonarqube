...
public class EmployeeControllerTest {
  ...
  public void findAllEmployee_expectSuccess() throws Exception {
    ...
    Mockito.when(employeeService.listEmployee())
        .thenReturn(
            ResponseEntity.ok()
                .body(
                    BaseResponse.builder()
    ...
    Mockito.when(employeeService.updateEmployee(any()))
        .thenReturn(
            ResponseEntity.ok()
                .body(
                    BaseResponse.builder()
                        .message(ConstantMess.KEY_SUCCESS_UPDATE)
                        .data(getEmployee())
                        .build()));
    ...
    mockMvc
        .perform(
            post("/employee/update")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"id\":4,\"nik\":\"AB1234\",\"first_name\":\"Dede\",\"last_name\":\"Hamzah\",\"gender\":\"laki-laki\",\"salary\":1000000,\"title\":\"Programmer\"}"))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/employee/create")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"nik\":\"AB1234\",\"first_name\":\"Dede\",\"last_name\":\"Hamzah\",\"gender\":\"laki-laki\",\"salary\":1000000,\"title\":\"Programmer\"}"))
        .andExpect(status().isOk())
    ...
    Mockito.when(employeeService.deleteEmployee(any()))
        .thenReturn(
            ResponseEntity.ok()
                .body(
                    BaseResponse.builder()
                        .message(ConstantMess.KEY_SUCCESS_DELETE)
                        .data(1L)
                        .build()));

    mockMvc
        .perform(
            post("/employee/delete").contentType(MediaType.APPLICATION_JSON).content("{\"id\":1}"))
        .andExpect(status().isOk())
  ...
  public void deleteEmployee_expectError() throws Exception {
    ...
    mockMvc
        .perform(
            post("/employee/delete").contentType(MediaType.APPLICATION_JSON).content("{\"id\":3}"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.message", is(ConstantMess.KEY_DATA_NOT_FOUND)));
  }
    ...
    mockMvc
        .perform(
            post("/employee/create")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"id\":1,\"nik\":\"AB1234\",\"first_name\":\"Dede\",\"last_name\":\"Hamzah\",\"gender\":\"laki-laki\",\"salary\":1000000,\"title\":\"Programmer\"}"))
